...
def is_media_file(filename):
    media_extensions = ('.mp3', '.mp4', '.mov', '.mkv', '.ogg', '.wav')
    return any(filename.lower().endswith(extension) for extension in media_extensions)
        ...
        for filename in filenames:
            ...
            if is_media_file(filename):
                ...
                if checksum in files_map:
                    ...
                    print(f"{file_path}: skipped (not in date_file)")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Check and update file modification dates for media files in a given folder and subfolders')
    ...
    print(f"Checked and updated media files in {args.dir_in} using dates from {date_file}")
